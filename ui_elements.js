function UiElementsMain(buildings) {
	addModal();
	addBuildingModal();
	...
	$("#lssp-button").on("click", () => lsspOpenModal(buildings));
	$("#lssp-building-modal-form").submit((event) => buildingModalOptions(event));
}

////////////////////////////////////////////////////////////////

function buildingModalOptions(event) {
	...
	if (
		...
		// Delete Building
		window.lssp.deleteElementById(window.lssp.db, building.id);
		//location.reload();
	} else if (
		event.originalEvent.submitter ==
		...
		// Edit Building
		lsspOpenEditModal(building);
	}
}

function lsspOpenModal(buildings) {
	...
	buildings.sort().forEach((b) => {
		$("#lssp-modal-dash-table-body").append(`
            <tr>
                ...
                <td>
                    <a id="lssp-modal-dash-table-body-link">
                        ${b.name}
                    </a>
                </td>
        ...
        `);
		let Buttons = document.querySelectorAll(`#lssp-modal-dash-table-body-link`);
...
function lsspOpenEditModal(building) {
...
function planNewBuilding() {
	...
	document
		...
		.addEventListener("click", saveNewBuilding, false);
...
function addLeafletButtons() {
	...
	document
		...
		.addEventListener("click", planNewBuilding, false);
